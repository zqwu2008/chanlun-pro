...
def xg_day_bi2_30m_bi1(code: str, mk_datas: MarketDatas):
    """
    选择同时满足以下条件的股票：
    1. 日线级别笔出现2买
    2. 30分钟级别笔出现1买
    ...
    """
    # 检查日线级别
    day_data = mk_datas.get_cl_data(code, "d")
    if len(day_data.get_bis()) == 0:
        return None
    
    day_bi = day_data.get_bis()[-1]
    # 检查是否有2买
    if not day_bi.mmd_exists(["2buy"], "|"):
        return None

    # 检查30分钟级别
    min30_data = mk_datas.get_cl_data(code, "30m")
    if len(min30_data.get_bis()) == 0:
        return None
    
    min30_bi = min30_data.get_bis()[-1]
    # 检查是否有1买
    if not min30_bi.mmd_exists(["1buy"], "|"):
        return None

    # 同时满足条件，返回结果
    return {
        "code": code,
        "msg": f"日线笔买点【{day_bi.line_mmds('|')}】 30分钟笔买点【{min30_bi.line_mmds('|')}】",
    }
...
def xg_day_bi_1buy(code: str, mk_datas: MarketDatas):
    """
    选择日线级别笔出现1买的股票
    ...
    # 检查是否有1买
    if day_bi.mmd_exists(["1buy"], "|"):
        return {
            "code": code,
            "msg": f"日线笔买点【{day_bi.line_mmds('|')}】",
        }
